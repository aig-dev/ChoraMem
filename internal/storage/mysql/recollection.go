...
func applyRecollectionKeep(ctx context.Context, tx *transaction, target *activeRecollection, refs []string) (bool, error) {
...
func insertRecollectionBasis(
	...
	for _, ref := range sortedRefs(refs) {
		tag, err := tx.Exec(ctx, `
			...
			ON DUPLICATE KEY UPDATE role = recollection_basis_links.role
