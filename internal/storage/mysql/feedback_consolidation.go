	...
	for _, episode := range episodes {
		rows, err := querier.Query(ctx, `
			...
			JOIN memory_delivery_receipts AS delivery
			 ...
			 AND JSON_CONTAINS(
				delivery.delivered_memory_refs,
				JSON_QUOTE(CONVERT(memory_item.memory_ref USING utf8mb4))
			 )
...
func applyReenactment(ctx context.Context, tx *transaction, owner ownerScope, target *feedbackTarget, refs []string) (bool, error) {
	...
	for _, episodeRef := range sortedRefs(refs) {
		...
		tag, err := tx.Exec(ctx, `
			...
			ON DUPLICATE KEY UPDATE role = seed_basis_links.role
...
func applyInhibition(ctx context.Context, tx *transaction, owner ownerScope, target *feedbackTarget, refs []string) (bool, error) {
...
func applyRevision(ctx context.Context, tx *transaction, owner ownerScope, jobRef string, target *feedbackTarget, change consolidation.Change) (string, bool, error) {
...
func insertEpisodeBasis(ctx context.Context, tx *transaction, tenantRef, versionRef string, refs []string, role string) (bool, error) {
	applied := false
	for _, ref := range refs {
		tag, err := tx.Exec(ctx, `
			INSERT INTO seed_basis_links (tenant_ref, seed_version_ref, episode_ref, role)
...
func insertOutcomeBasis(ctx context.Context, tx *transaction, tenantRef, versionRef string, refs []string, role string) (bool, error) {
	applied := false
	for _, ref := range refs {
		tag, err := tx.Exec(ctx, `
			INSERT INTO seed_outcome_basis_links (tenant_ref, seed_version_ref, outcome_event_ref, role)
			VALUES ($1, $2, $3, $4)
			ON DUPLICATE KEY UPDATE role = seed_outcome_basis_links.role
		`, tenantRef, versionRef, ref, role)
		if err != nil {
