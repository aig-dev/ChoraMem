...
func (store *Store) RecordMemoryDelivery(ctx context.Context, delivery ledger.MemoryDelivery) (ledger.MemoryDeliveryReceipt, error) {
	...
	if err := tx.acquireLock(ctx, feedbackRequestLockKey("delivery", delivery.Scope.TenantRef, delivery.IdempotencyKey)); err != nil {
...
func (store *Store) ReportOutcome(ctx context.Context, report ledger.OutcomeReport) (ledger.OutcomeReceipt, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	if err := tx.acquireLock(ctx, feedbackRequestLockKey("outcome", report.Event.Scope.TenantRef, report.IdempotencyKey)); err != nil {
...
func loadMemoryDeliveryReceipt(ctx context.Context, tx *transaction, tenantRef, idempotencyKey string, requestHash [sha256.Size]byte) (ledger.MemoryDeliveryReceipt, bool, error) {
...
func loadDeliverableMemoryRefs(ctx context.Context, tx *transaction, delivery ledger.MemoryDelivery) (map[string]string, error) {
...
func loadOutcomeReceipt(ctx context.Context, tx *transaction, report ledger.OutcomeReport, requestHash [sha256.Size]byte) (ledger.OutcomeReceipt, bool, error) {
...
func validateOutcomeBindings(ctx context.Context, tx *transaction, report ledger.OutcomeReport, deliveryRefs, relatedRefs []string) error {
		...
		if err := tx.QueryRow(ctx, `
			SELECT EXISTS (
				...
				WHERE tenant_ref = $1
				  ...
				  AND source_group_ref = $7
				  AND role = 'agent_act'
				  AND JSON_CONTAINS($8, JSON_QUOTE(CONVERT(source_event_ref USING utf8mb4)))
			)
		`,
			report.Event.Scope.TenantRef,
			report.Event.Scope.Kind,
...
func rejectRetroactiveMemoryDelivery(ctx context.Context, tx *transaction, delivery ledger.MemoryDelivery) error {
...
func lockFeedbackRun(ctx context.Context, tx *transaction, scope ledger.Scope, runRef string) error {
	if err := tx.acquireLock(ctx, feedbackRunLockKey(scope, runRef)); err != nil {
