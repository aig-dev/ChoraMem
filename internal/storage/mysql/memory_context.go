...
func (store *Store) SelectMemory(ctx context.Context, request selection.SelectRequest) (selection.MemoryContext, error) {
	...
	if err := tx.acquireLock(ctx, memoryRunLockKey(request.Scope, request.RunRef)); err != nil {
	...
	for _, owner := range selectionOwners(request.Scope) {
		if err := tx.acquireLock(ctx, consolidationOwnerLockKey(owner)); err != nil {
...
func loadSituationQueries(ctx context.Context, tx *transaction, scope ledger.Scope, refs []string) ([]selection.Query, error) {
...
func loadSelectionCandidates(ctx context.Context, tx *transaction, scope ledger.Scope, perOwnerKindLimit int) ([]selection.Candidate, error) {
...
func loadRecollectionCandidates(ctx context.Context, tx *transaction, scope ledger.Scope, limit int, refs []string) ([]selection.Candidate, error) {
	rows, err := tx.Query(ctx, `
		WITH ranked_versions AS (
			...
			WHERE recollection.tenant_ref = $1
			  ...
			  AND ($6 IS NULL OR JSON_CONTAINS(
				$6, JSON_QUOTE(CONVERT(version.recollection_version_ref USING utf8mb4))
			  ))
			  AND (
...
func loadDispositionCandidates(ctx context.Context, tx *transaction, scope ledger.Scope, limit int, refs []string) ([]selection.Candidate, error) {
	rows, err := tx.Query(ctx, `
		WITH ranked_versions AS (
			SELECT
			...
			WHERE seed.tenant_ref = $1
			  ...
			  AND version.status = 'active'
			  AND ($6 IS NULL OR JSON_CONTAINS(
				$6, JSON_QUOTE(CONVERT(version.seed_version_ref USING utf8mb4))
			  ))
			  AND (
...
func loadIndexedSelectionCandidates(ctx context.Context, tx *transaction, indexed []scopedIndexCandidate) ([]selection.Candidate, error) {
...
func loadEpisodeLinkedMemoryRefs(ctx context.Context, tx *transaction, owner ownerScope, episodeRefs []string) ([]string, []string, error) {
	recollectionRows, err := tx.Query(ctx, `
		...
		WHERE episode.tenant_ref = $1
		  ...
		  AND JSON_CONTAINS($5, JSON_QUOTE(CONVERT(episode.episode_ref USING utf8mb4)))
...
func scanRefs(rows rows) ([]string, error) {
	defer rows.Close()
	refs := make([]string, 0)
	for rows.Next() {
		var ref string
...
func loadMemoryContext(ctx context.Context, tx *transaction, request selection.SelectRequest, requestHash [sha256.Size]byte) (selection.MemoryContext, bool, error) {
