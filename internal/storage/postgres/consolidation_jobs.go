...
func enqueueConsolidationEvidence(
	...
	if _, err := tx.Exec(ctx, "SELECT pg_advisory_xact_lock($1)", consolidationCollectionLockKey(owner)); err != nil {
...
func enqueueEpisodeState(ctx context.Context, tx pgx.Tx, scope ledger.Scope, episodeRef string) error {
	if episodeRef == "" {
		return nil
	}
...
func enqueueRunEpisodes(ctx context.Context, tx pgx.Tx, scope ledger.Scope, runRef string) error {
	rows, err := tx.Query(ctx, `
		SELECT episode_ref
		FROM episodes
		WHERE tenant_ref = $1
		...
		WHERE job.completed_at IS NULL
		  ...
		  AND (
			job.window_hash IS NOT NULL OR (
				(job.updated_at <= $1 OR cardinality(job.episode_refs) >= $2) AND
				cardinality(job.episode_refs) >= 1
			)
		  )
...
func (store *Store) CompleteConsolidationJob(ctx context.Context, jobRef, leaseToken string) error {
	tag, err := store.pool.Exec(ctx, `
		UPDATE consolidation_jobs
		SET completed_at = now(), lease_token = '', lease_until = NULL, updated_at = now()
...
func (store *Store) RetryConsolidationJob(ctx context.Context, jobRef, leaseToken string, notBefore time.Time) error {
	tag, err := store.pool.Exec(ctx, `
		UPDATE consolidation_jobs
		SET not_before = $3, lease_token = '', lease_until = NULL, updated_at = now()
		WHERE job_ref = $1 AND lease_token = $2 AND completed_at IS NULL
...
func prependPreviousOverlap(ctx context.Context, tx pgx.Tx, owner ownerScope, current []string, limit int) ([]string, []string, error) {
	if limit <= 0 {
