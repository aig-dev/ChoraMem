...
func TestDirectAdaptationPersistsOnlyVersionsAndEpisodeBasis(t *testing.T) {
	...
	first := harness.materializeEpisode(t, "adapt-initial", "session-adapt-initial")
	second := harness.materializeEpisode(t, "adapt-correction", "session-adapt-correction")
	third := harness.materializeEpisode(t, "adapt-same-text", "session-adapt-same-text")
	...
	for index, episode := range []string{first, second, third} {
		...
		worker := &scriptedWorker{taggedText: fmt.Sprintf("TARGET\n%s\nAPPLICATION\nRELATION\nCHANGE\nADAPT\n%s\nBASIS\n%s", target, text, episode)}
		window := consolidation.Window{JobRef: fmt.Sprintf("adapt-provenance-%d", index), EpisodeRefs: []string{episode}}
		result, err := harness.store.ConsolidateWindow(context.Background(), window, worker)
		if err != nil {
			t.Fatal(err)
		}
		if index < 2 {
		...
		retry, err := harness.store.ConsolidateWindow(context.Background(), window, &scriptedWorker{taggedText: "must not execute"})
	...
	for table, want := range map[string]int{
		...
		if count := harness.count(t, table, "TRUE", nil); count != want {
	...
	for role, want := range map[string]int{"formation": 1, "revision": 1, "reenactment": 0, "inhibition": 0} {
		if count := harness.count(t, "seed_basis_links", "role = $2", role); count != want {
			t.Fatalf("%s Basis = %d; want %d", role, count, want)
		}
	}
	if count := harness.count(t, "seed_versions", "status = $2", "superseded"); count != 1 {
		t.Fatalf("superseded versions = %d; want 1", count)
	}
}
